...
/**
 * Lê uma imagem ou PDF de comprovante ou nota fiscal e extrai os dados estruturados via SDK oficial @google/generative-ai
 */
export async function parseReceiptWithGemini(base64Data: string, mimeType: string): Promise<ExtractedReceipt> {
  const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
  if (!apiKey || apiKey === 'COLE_SUA_CHAVE_AQUI' || apiKey.trim() === '') {
    throw new Error('API Key ausente. Configure a variável VITE_GEMINI_API_KEY no arquivo .env');
  }
...
Retorne EXCLUSIVAMENTE o objeto JSON válido, sem texto explicativo adicional.`;
  ...
  // base64Data é APENAS a string pura sem prefixo data:...
